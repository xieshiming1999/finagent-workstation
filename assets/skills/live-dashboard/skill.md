...
Score the artifact on the five dimensions inherited from `$SKILL_DIR/../critique/`:
...
- In FinAgent Workstation, write dashboard pages under `{{DATA_DIR}}/memory/pages/` unless the user
  or current file explicitly says otherwise. Use `UIControl(openPage)` with the
  concrete `{{DATA_DIR}}/memory/pages/<name>.html` path. `{{DATA_DIR}}` is
  replaced by the runtime before this skill is shown to the agent; do not
  reconstruct project data paths by hand.

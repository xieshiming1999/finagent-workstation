...
export function customStrategyHelp(): string {
  ...
  const text = [
    ...
    '',
    `Executable v1 supports indicators ${[...executableIndicators].sort().join(', ')}; operators > >= < <= crosses_above crosses_below; exits stop_loss_pct/take_profit_pct/trailing_stop_pct/max_drawdown_stop_pct/atr_stop_loss/time_stop_bars; full_capital, fixed_fraction, risk_per_trade, or capped kelly_fraction sizing; rank metrics score, total_return_pct, sharpe_ratio, max_drawdown_pct, trade_count, relative_strength_pct, rps; portfolio draft controls rebalanceInterval, maxPositionWeight, minScore, maxPairwiseCorrelation, and StrategySpec.risk.maxDrawdownPct.`,
    `Fund StrategySpec requires assetClass:"fund" or market:"fund". Fund observation indicators: ${fundStrategyIndicatorCatalog.join(', ')}. Use custom_strategy_observe for current fund signal evidence and custom_strategy_fund_backtest for NAV/yield period evidence. Provide fundRows from query_fund_nav or query_fund_money_yield. Do not send fund specs to custom_strategy_backtest.`,
    ...
    `Indicator catalog categories: ${Object.keys(groupedIndicatorCatalog).sort().join(', ')}.`,
    `Indicator catalog: ${JSON.stringify(indicatorHelpCatalog)}.`,
  ...
  return JSON.stringify({
    ...
    executableV1: {
      indicators: [...executableIndicators].sort(),
      indicatorCatalog: indicatorHelpCatalog,
      indicatorCategories: Object.keys(groupedIndicatorCatalog).sort(),
      indicatorCatalogByCategory: groupedIndicatorCatalog,
      stockExample: {
    ...
    fundObservationV1: {
      ...
      indicatorCatalog: fundIndicatorHelpCatalog,
      indicatorCategories: Object.keys(fundIndicatorCatalogByCategory()),
      indicatorCatalogByCategory: fundIndicatorCatalogByCategory(),
      ordinaryFundExample: {
        name: 'fund_nav_dca',
        assetClass: 'fund',
        market: 'fund',

...
export default function SessionPanel() {
  ...
  useEffect(() => {
    window.agent?.sessions().then((list: SessionEntry[]) => {
      const rows = list ?? []
      setSessions(rows)
  ...
  const resumeSelectedSession = async (session: SessionEntry) => {
    if (session.isCurrent) return
    setResuming(true)
    try {
      await window.agent?.resume(session.path)
      useAgentStore.getState().restoreSession()
      setSelected(session)
    } finally {
      setResuming(false)
    }
  ...
  const filteredSessions = sessions.filter((session) => {
    ...
    if (!q) return true
    return [session.title, session.firstPrompt, session.name, session.path].some((value) => String(value ?? '').toLowerCase().includes(q))
  })
  ...
  return (
    <div className="h-full flex flex-col theme-bg theme-text-secondary">
      <div className="shrink-0 border-b theme-border p-2 space-y-2">
        <div>
          <div className="text-xs theme-text font-medium">{t('session')}</div>
          <div className="text-[10px] theme-text-tertiary">{t('sessionPanelHelp')}</div>
        </div>
        <div className="flex gap-1">
          <input
            ...
            className="min-w-0 flex-1 theme-bg-secondary border theme-border rounded px-2 py-1 text-xs theme-text-secondary focus:outline-none focus:border-[#2962ff]"
          />
        </div>
        {sessions.length === 0 ? (
          <div className="text-xs text-center theme-text-tertiary py-4">{t('noArchivedSessions')}</div>
        ) : (
          <div className="max-h-44 overflow-y-auto space-y-1">
            {filteredSessions.map((session) => {
              ...
              return (
                <div
                  key={session.path}
                  className={`px-2 py-1.5 rounded text-xs ${active ? 'theme-bg-tertiary' : 'hover:theme-bg-secondary'}`}
                >
                  <div className="flex items-start gap-2">
                    ...
                    {session.isCurrent ? (
                      <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded border theme-border theme-text-tertiary uppercase tracking-wide">
                        {t('active')}
                      </span>
                    ) : (
                      <button
                        onClick={() => {
                          void previewController.select(session)
                          void resumeSelectedSession(session)
                        }}
                        disabled={resuming}
                        className="shrink-0 text-[10px] px-1.5 py-0.5 rounded border theme-border theme-text hover:theme-bg-secondary disabled:opacity-40"
                      >
                        {resuming && selected?.path === session.path ? t('loading') : t('resume')}
                      </button>
                    )}
                  </div>
                </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="text-xs theme-text font-medium truncate">{preview?.title || (selected ? displayTitle(selected) : t('session'))}</div>
